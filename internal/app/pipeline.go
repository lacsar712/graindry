...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.dehumid.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.dehumid.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.dehumid.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *moisture.ProfileManager, windows *moisture.HoldWindowManager) *moisture.HoldWindowEvaluator {
	return moisture.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *dehumid.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *dehumid.Controller {
	c := dehumid.NewController(bank, maxDelta)
